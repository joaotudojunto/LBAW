...
document.querySelectorAll(".follow_stats").forEach((element) => {
    element.addEventListener("click", function(event) {
        ...
        if (classList.contains("button-tags")) {
            handleFollowedtagsClick(event.target)
        }
    });
...
function handleFollowedtagsClick(button){
    const route = "/api/member/" + button.getAttribute("data-id") + "/followedtags";
    let request = {};
    sendAjaxRequest("GET", route, request, (response) => {
        const json_data = JSON.parse(response)
        let modal_followedtags = document.querySelector(".container-tag");
        let htmlFollowedtags = json_data;
        modal_followedtags.innerHTML = '';
        htmlFollowedtags.forEach(element => {
            modal_followedtags.innerHTML += element;
        });
    })
}
