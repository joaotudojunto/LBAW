<div class="row bg-white p-3 g-0 mb-3 rounded border tag-card" data-id={{$tag->id}}>
    <div class="row align-items-center">
        <img src="{{ asset('storage/assets/letters/'.strtoupper(substr($tag->name, 0, 1)).'.png') }}" alt="tag image" class="col-2 flex-shrink-0 rounded-circle"
             style="width:90px;height:auto;">
        <div class="col-md-10 col-8 d-flex">
            <div>
                <h4><a href="{{ route('tag', ['tag' => $tag->name]) }}">{{$tag->name}}</a></h4>
                <p class="h6 fw-normal">{{$tag->followers->count()}} Followers</p>
            </div>
        </div>
    </div>
    ...
    <div class="d-flex justify-content-between">
        <button type="button" class="border-0 text-danger bg-body" data-bs-toggle="modal"
            data-bs-target="#modalReports{{$tag->id}}"><h4 class="text-danger">{{$tag->reports->count()}} reports</h4>
        </button>
        <div class="col-2 ">
...
<div class="modal fade" id="modalReports{{$tag->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
